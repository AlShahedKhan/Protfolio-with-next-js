...
export default function ProjectForm({
  ...
  const onSubmit = handleSubmit((values) => {
    startTransition(async () => {
      ...
      try {
        payload = buildAdminProjectPayload(values);
      } catch {
      ...
      if (!response.ok) {
        ...
        setFieldError('image_url', 'image_url');
  ...
  const summary = isEditMode
    ? 'Update an existing Laravel project record through the secure Next.js admin route.'
    : 'This form posts through your Next.js admin route, which forwards the request to Laravel with the secure admin token.';
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <input
                id="image_url"
                {...register('image_url')}
                placeholder="https://api.example.com/storage/projects/orfa-ai.jpg"
                className={fieldClassName}
              />
              {errors.image_url && (
                <p className="text-sm text-red-300">{errors.image_url.message}</p>
              )}
            </div>
